...
function App() {
  ...
  const visibleExpanses = selectedCategory ? expenses.filter((e) => e.category === selectedCategory): expenses;

  return (
    <>
     <div className="mb-3">
       <ExpenseForm onSubmit={expense => setExpanses([...expenses, {...expense, id: expenses.length+ 1}])}/>
     </div>
     <div className="mb-3">
       <ExpanseFilter onSelectCategory={category => setSelectedCategory(category)}/>
     </div>
     <ExpanseList  expenses={visibleExpanses} onDelete={(id) => setExpanses(expenses.filter((e) => e.id !== id))}/>
    </>
  )
}
